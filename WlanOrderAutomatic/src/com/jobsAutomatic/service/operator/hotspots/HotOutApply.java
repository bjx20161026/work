...
public class HotOutApply {
	/*
	 * �ȵ������˷�
	 */
	public void OutApply(String hotspotid){
		...
		jdbcTemplate.update("insert into RES.PRM_AUDIT_REQUEST (REQ_NO, REQ_TYPE, TARGET_TYPE, OBJECT_ID, EMP_NO, REQ_TIME, REQ_CAUSE, ATTACHMENT, STATUS ) values (null, 2, 2, ?, '20000000', sysdate, ?, ?, 0)",hotspotid,"测试",null);
		jdbcTemplate.update("update prm_wlan_hotspot set audit_status = 5,ENTRY_DATE=sysdate WHERE hotspot_id = ? ",hotspotid);
		jdbcTemplate.update("update prm_device set audit_status = 5,ENTRY_DATE=sysdate where device_id in ( select device_id from prm_device_ap WHERE hotspot_id = ? )",hotspotid);
